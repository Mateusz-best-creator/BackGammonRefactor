...
bool check_dices_2(Player* player_AI)
{
	if (player_AI->dice1 == 0 && player_AI->dice2 == 0) { return true; }
}

void count_pawns_2(int& pawns_on_bar, Board* table_s, char AI_pawn_char)
{
...
void loop_through_columns_2(Board* table_s, int dice_value, char AI_pawn_char, Player* player_AI, int player_AI_pawns_on_bar)
{
...
void move_to_the_board_2(Board* table_s, int player_AI_pawns_on_bar, int dice_value, Player* player_AI, char AI_pawn_char)
{
	...
	if (player_AI_pawns_on_bar > 0)
	{
		...
		{
			loop_through_columns_2(table_s, dice_value, AI_pawn_char, player_AI, player_AI_pawns_on_bar);
		}
		print_table(table_s);
	}
}
...
{
	if (check_dices_2(player_AI)) { return; }
	dice_value--;
	int player_AI_pawns_on_bar = 0;
	count_pawns_2(player_AI_pawns_on_bar, table_s, AI_pawn_char);

	move_to_the_board_2(table_s, player_AI_pawns_on_bar, dice_value, player_AI, AI_pawn_char);
}