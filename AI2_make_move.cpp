...
void c_from_bar_2(Board* table_s, Player* player_AI, char AI_pawn_char)
{
	/*
...
bool c_if_home_2(Board* table_s, Player* player_AI, char AI_pawn_char, int& i, int& j, int& dice_sum)
{
...
void loop_through_board_2(Board* table_s, Player* player_AI, char AI_pawn_char, int dice_sum)
{
	for (int i = NUMBER_OF_COLUMNS - 1; i >= 0; i--)
	{
		for (int j = NUMBER_OF_ROWS_IN_COLUMN - 1; j >= 0; j--)
		{
			c_from_bar_2(table_s, player_AI, AI_pawn_char);
			if (player_AI->dice1 == 0 && player_AI->dice2 == 0) { return; }

			// Update dice_sum variable
			dice_sum = player_AI->dice1 + player_AI->dice2;
			if (c_if_home_2(table_s, player_AI, AI_pawn_char, i, j, dice_sum))
				return;
